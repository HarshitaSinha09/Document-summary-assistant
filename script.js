...
let selectedLength = null; // Changed to null so no length is selected by default
...
initialLengthSelector.querySelectorAll('.length-option').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        checkCanSummarize(); // Call a new function to check if button should be enabled
    });
});
...
// New function to check if both a file and a length are selected
function checkCanSummarize() {
    if (selectedFile && selectedLength) {
...
async function handleInitialGeneration() {
    if (!selectedFile || !selectedLength) {
        showError('Please upload a document and select a summary length.');
        return;
    }
...
function resetToUpload() {
    selectedFile = null;
    selectedLength = null; // Resetting selected length on new document
    documentText = '';
    fileSelector.value = '';
    ...
    summarizeButton.disabled = true;
    initialLengthSelector.querySelector('.active')?.classList.remove('active'); // Remove active class from buttons
    hideError();
}
